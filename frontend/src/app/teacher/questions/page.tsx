...
const TYPE_LABELS: Record<string, string> = {
  SingleChoice: "1 đáp án", MultipleChoice: "Nhiều đáp án", TrueFalse: "Đúng/Sai",
  FillBlank: "Điền vào chỗ trống", Matching: "Ghép đôi", Ordering: "Sắp xếp",
};
const SKILL_LABELS: Record<string, string> = {
  Reading: "Đọc", Listening: "Nghe", Speaking: "Nói",
  Writing: "Viết", Grammar: "Ngữ pháp", Vocabulary: "Từ vựng", Mixed: "Tổng hợp",
};
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 680, margin: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h3 style={{ fontSize: 18, fontWeight: 700, color: "#111827" }}>
            {initial.content ? "Chỉnh sửa câu hỏi" : "Tạo câu hỏi mới"}
          </h3>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Nội dung câu hỏi *</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Loại câu hỏi</label>
              <select value={form.type} onChange={(e) => setF("type", e.target.value)}
                style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #D1D5DB", fontSize: 13, background: "#fff" }}>
                {Q_TYPES.map((t) => <option key={t} value={t}>{TYPE_LABELS[t]}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={form.skillType} onChange={(e) => setF("skillType", e.target.value)}
                style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: "1px solid #D1D5DB", fontSize: 13, background: "#fff" }}>
                {SKILL_TYPES.map((t) => <option key={t} value={t}>{SKILL_LABELS[t] ?? t}</option>)}
              </select>
            </div>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Độ khó</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Điểm</label>
          ...
          {showOptions && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>Đáp án (click để chọn đúng)</label>
                <button onClick={addOption} style={{ fontSize: 12, color: MLS_NAVY, background: "none", border: "none", cursor: "pointer", fontWeight: 600 }}>+ Thêm</button>
              </div>
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Giải thích (hiển thị sau khi nộp)</label>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 24, justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{ padding: "10px 20px", borderRadius: 8, border: "1px solid #D1D5DB", background: "#fff", cursor: "pointer", fontWeight: 600, fontSize: 14 }}>Hủy</button>
  ...
  async function handleSave(form: QuestionForm) {
    try {
      if (modal === "create") {
        ...
        showMsg("success", "Tạo câu hỏi thành công");
      } else if (editTarget) {
        await updateQuestion({ id: editTarget.id, ...form }).unwrap();
        showMsg("success", "Đã cập nhật câu hỏi");
      }
      setModal("none");
      setEditTarget(null);
    } catch {
      showMsg("error", modal === "create" ? "Tạo câu hỏi thất bại" : "Cập nhật thất bại");
    }
  }
  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "#111827", marginBottom: 4 }}>Ngân hàng câu hỏi</h1>
          <p style={{ fontSize: 14, color: "#6B7280" }}>{total} câu hỏi</p>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "#F8FAFC", borderBottom: "2px solid #E5E7EB" }}>
              {["Câu hỏi", "Loại", "Kỹ năng", "Độ khó", "Điểm", "Ngày tạo", "Thao tác"].map((h) => (
                <th key={h} style={{ padding: "12px 14px", textAlign: "left", fontSize: 12, fontWeight: 700, color: "#6B7280", textTransform: "uppercase" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {isFetching && questions.length === 0 ? (
              ...
              <tr><td colSpan={7} style={{ textAlign: "center", padding: 40, color: "#9CA3AF" }}>Chưa có câu hỏi nào</td></tr>
            ) : (
              questions.map((q) => (
                <tr key={q.id} style={{ borderBottom: "1px solid #F3F4F6" }}
                  ...
                  <td style={{ padding: "13px 14px" }}>
                    <span style={{ fontSize: 12, background: DIFF_BG[q.difficulty], color: DIFF_COLOR[q.difficulty], borderRadius: 4, padding: "2px 8px", fontWeight: 600 }}>{q.difficulty}</span>
                  </td>
                  <td style={{ padding: "13px 14px", fontSize: 13, color: "#374151" }}>{q.score}</td>
                  <td style={{ padding: "13px 14px", fontSize: 12, color: "#9CA3AF" }}>{new Date(q.createdAt).toLocaleDateString("vi-VN")}</td>
                  <td style={{ padding: "13px 14px" }}>
      ...
      {confirmDelete && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }}>
          <div style={{ background: "#fff", borderRadius: 14, padding: 24, maxWidth: 360, width: "90%" }}>
            <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>Xác nhận xóa?</h3>
            <p style={{ color: "#6B7280", fontSize: 14, marginBottom: 20 }}>Câu hỏi sẽ bị xóa vĩnh viễn và không thể khôi phục.</p>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => setConfirmDelete(null)} style={{ flex: 1, padding: "9px 0", borderRadius: 8, border: "1px solid #D1D5DB", background: "#fff", cursor: "pointer", fontWeight: 600 }}>Hủy</button>
